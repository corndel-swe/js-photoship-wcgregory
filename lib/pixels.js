...
export function adjustBrightness(rgb, brightness) {
  ...
  // using a multipler for this but will check with PDE
  const brighter = Array()
  const maxColourValue = 255

  for (let indx = 0; indx < rgb.length; indx++) {
    brighter[indx] = rgb[indx] + Number(brightness)
    if (brighter[indx] > maxColourValue) {
      brighter[indx] = maxColourValue
    }
  }
  return brighter

  // for (let colour of rgb) {
  //   brighter[index] = rgb[colour] * brightness
  //   if (brighter[index] > maxColourValue) {
  //     brighter[index] = maxColourValue
  //  }
  //   index++
  // }
  // return brighter
