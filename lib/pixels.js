...
export function blackAndWhite(rgb) {
  ...
  for (let indx = 0; indx < rgb.length; indx++) {
    blackWhite[indx] = [value]
  }
  // rewritten with a for loop instead
  // while (rgb.length !== 0) {
  //  rgb.pop()
  //  blackWhite.push([value])
  //}
  return blackWhite
}
...
export function colorChannel(rgb, color) {
  ...
  const colourChannel = [0, 0, 0]

  if (color === 'r') {
    colourChannel[0] = rgb[0]
  } else if (color === 'g') {
    colourChannel[1] = rgb[1]
  } else if (color === 'b') {
    colourChannel[2] = rgb[2]
  }
...
export function flipReverseAndBrighten(rgb, brightness) {
  // TODO: reverse the colour values, except the middle value
  // The colour values will become reversed, i.e. 'RGB to BGR'
  // e.g. [107, 43, 198] => [198, 43, 107]
  // TODO: add the value of 'brightness' to every element in rgb
